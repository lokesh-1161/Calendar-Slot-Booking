...
<head>
  <meta charset="UTF-8"/>
  <title>HTML5 candiate Appointment Scheduling (JavaScript/PHP)</title>
...
<div class="main">
  ...
  <div>
    ...
    <div class="column-main">
      <div class="space">
        <select id="candiate" name="candiate">
          <?php
          foreach ($db->query('SELECT * FROM candiate ORDER BY candiate_name') as $item) {
            echo "<option value='" . $item["candiate_id"] . "'>" . $item["candiate_name"] . "</option>";
          }
          ?>
        </select>
      </div>
...
//                var start = nav.visibleStart() > new DayPilot.Date() ? nav.visibleStart() : new DayPilot.Date();
    ...
    var params = {
      candiate: $("#candiate").val(),
      start: start.toString(),
      end: nav.visibleEnd().toString()
    };

    $.post("backend_events_candiate.php", JSON.stringify(params), function (data) {
      if (day) {
