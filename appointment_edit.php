...
<div style="padding:10px">
  ...
  <div class="space">
    <div>candiate:</div>
    <div>
      <select id="resource" name="resource" disabled ng-model="appointment.candiate">
        <?php
        foreach ($db->query('SELECT * FROM [candiate] ORDER BY [candiate_name]') as $item) {
          $selected = "";
          if ($event["candiate_id"] == $item["candiate_id"]) {
            $selected = " selected";
          }
          echo "<option value='" . $item["candiate_id"] . "'" . $selected . ">" . $item["candiate_name"] . "</option>";
        }
        ?>
      </select>
    </div>
